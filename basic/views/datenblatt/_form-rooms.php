            ...
            <td class="vcenter">
                <?php
                    ...
                    if (! $modelRoom->isNewRecord) {
                        echo Html::activeHiddenInput($modelRoom, "[{$indexZahlung}][{$indexRoom}]id");
                    }
                ?>
                <?= $form->field($modelRoom, "[{$indexZahlung}][{$indexRoom}]description")->label(false)->textInput(['maxlength' => true]) ?>
            </td>
